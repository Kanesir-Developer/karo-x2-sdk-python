...
from .messages import RobotStatus, SafetyEvent, TaskEvent
...
from .topic import TopicDescriptor
...
class Robot:
    ...
    async def subscribe_task_events(
        self,
        on_data: Callable[[TaskEvent], Union[None, Awaitable[None]]],
        on_status: Optional[StatusCallback] = None,
    ) -> Subscription:
        return await self._impl.subscribe_task_events(on_data, on_status)

    async def subscribe_safety_events(
        self,
        on_data: Callable[[SafetyEvent], Union[None, Awaitable[None]]],
        on_status: Optional[StatusCallback] = None,
    ) -> Subscription:
        return await self._impl.subscribe_safety_events(on_data, on_status)
    ...
    async def create_navigation_task_to_marker(self, marker_id: str) -> CreateTaskResult:
        return await self._impl.create_navigation_task_to_marker(marker_id)

    async def create_navigation_task_to_pose(
        self, x: float, y: float, theta: float,
    ) -> CreateTaskResult:
        return await self._impl.create_navigation_task_to_pose(x, y, theta)

    async def cancel_task(self, task_id: str) -> TaskCommandResult:
        return await self._impl.cancel_task(task_id)

    async def pause_task(self, task_id: str) -> TaskCommandResult:
        return await self._impl.pause_task(task_id)

    async def resume_task(self, task_id: str) -> TaskCommandResult:
        return await self._impl.resume_task(task_id)
