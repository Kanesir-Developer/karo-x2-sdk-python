...
def encode_create_nav_task_marker(request_id: int, marker_id: str) -> bytes:
    req = _v1_tasks.CreateNavigationTaskRequest()
    req.marker_id = marker_id
    return _wrap_task_rpc(request_id, "tasks.create", req)
...
def encode_cancel_task(request_id: int, task_id: str) -> bytes:
    req = _v1_tasks.CancelTaskRequest()
    req.task_id = task_id
    return _wrap_task_rpc(request_id, "tasks.cancel", req)

def encode_pause_task(request_id: int, task_id: str) -> bytes:
    req = _v1_tasks.PauseTaskRequest()
    req.task_id = task_id
    return _wrap_task_rpc(request_id, "tasks.pause", req)

def encode_resume_task(request_id: int, task_id: str) -> bytes:
    req = _v1_tasks.ResumeTaskRequest()
    req.task_id = task_id
    return _wrap_task_rpc(request_id, "tasks.resume", req)
...
def decode_create_task_response(payload: bytes) -> Optional[str]:
    resp = _v1_tasks.CreateNavigationTaskResponse()
    try:
        resp.ParseFromString(payload)
    except Exception:
        return None
