...
class RobotImpl:
    ...
    async def _subscribe_internal(
        self, topic: str, desired_hz: float, frame_dispatcher, on_status,
    ) -> Subscription:
        slot = SubSlot(
        ...
        slot.frame_dispatcher = frame_dispatcher
    ...
    async def subscribe_robot_status(
        ...
        return await self._subscribe_internal(
            "robot.state", desired_hz, _frame_dispatcher, on_status)

    async def subscribe_task_events(
        self, on_data, on_status,
    ) -> Subscription:
        async def _frame_dispatcher(push: DecodedTopicPush) -> None:
            ev = pc.decode_task_event(push.payload)
            if ev is not None:
                ev.timestamp_ms = push.timestamp_ms
                await _maybe_await(on_data(ev))
        return await self._subscribe_internal(
            "tasks.events", 0.0, _frame_dispatcher, on_status)

    async def subscribe_safety_events(
        self, on_data, on_status,
    ) -> Subscription:
        async def _frame_dispatcher(push: DecodedTopicPush) -> None:
            ev = pc.decode_safety_event(push.payload)
            if ev is not None:
                ev.timestamp_ms = push.timestamp_ms
                await _maybe_await(on_data(ev))
        return await self._subscribe_internal(
            "robot.safety", 0.0, _frame_dispatcher, on_status)
    ...
    async def create_navigation_task_to_marker(self, marker_id: str):
        if self._state != ConnectionState.READY:
            from ..robot import CreateTaskResult
            return CreateTaskResult(accepted=False, code=ErrorCode.DISCONNECTED,
                                    message="Robot is not Ready")
        rid = self._next_rid()
        return await self._await_create_task(
            rid, pc.encode_create_nav_task_marker(rid, marker_id))

    async def create_navigation_task_to_pose(
        ...
        rid = self._next_rid()
        return await self._await_create_task(
            rid, pc.encode_create_nav_task_pose(rid, x, y, theta))

    async def _await_create_task(self, rid: int, bytes_: bytes):
        from ..robot import CreateTaskResult
        try:
            resp = await self._send_rpc_and_wait(
                rid, "move.create_task", bytes_, timeout=self.opts.estop_rpc_timeout)
            if not resp.success:
                return CreateTaskResult(accepted=False, code=resp.sdk_code,
                                        message=resp.message)
            if not resp.has_rpc:
                return CreateTaskResult(accepted=False,
                                        code=ErrorCode.INTERNAL_ERROR,
                                        message="missing rpc payload in response")
            if resp.rpc_code != ErrorCode.OK:
                return CreateTaskResult(accepted=False, code=resp.rpc_code,
                                        message=resp.rpc_message)
            task_id = pc.decode_create_task_response(resp.rpc_payload)
            if not task_id:
                return CreateTaskResult(
                    accepted=False, code=ErrorCode.INTERNAL_ERROR,
                    message="malformed CreateNavigationTask response")
            return CreateTaskResult(accepted=True, code=ErrorCode.OK,
                                    task_id=task_id)
        except asyncio.TimeoutError:
            return CreateTaskResult(accepted=False, code=ErrorCode.TIMEOUT,
                                    message="move.create_task timeout")
        except SdkException as e:
            return CreateTaskResult(accepted=False, code=e.code, message=str(e))

    async def cancel_task(self, task_id: str):
        return await self._task_command(
            "task_actions.cancel_task", pc.encode_cancel_task, task_id)

    async def pause_task(self, task_id: str):
        return await self._task_command(
            "task_actions.pause_task", pc.encode_pause_task, task_id)

    async def resume_task(self, task_id: str):
        return await self._task_command(
            "task_actions.resume_task", pc.encode_resume_task, task_id)

    async def _task_command(self, method: str, encoder, task_id: str):
        from ..robot import TaskCommandResult
        if self._state != ConnectionState.READY:
            return TaskCommandResult(ok=False, code=ErrorCode.DISCONNECTED,
                                     message="Robot is not Ready")
        rid = self._next_rid()
        bytes_ = encoder(rid, task_id)
        try:
            resp = await self._send_rpc_and_wait(
                rid, method, bytes_, timeout=self.opts.estop_rpc_timeout)
            if not resp.success:
                return TaskCommandResult(ok=False, code=resp.sdk_code,
                                         message=resp.message)
            if not resp.has_rpc:
                return TaskCommandResult(ok=False, code=ErrorCode.INTERNAL_ERROR,
                                         message="missing rpc payload in response")
            if resp.rpc_code != ErrorCode.OK:
                return TaskCommandResult(ok=False, code=resp.rpc_code,
                                         message=resp.rpc_message)
            return TaskCommandResult(ok=True, code=ErrorCode.OK)
        except asyncio.TimeoutError:
            return TaskCommandResult(ok=False, code=ErrorCode.TIMEOUT,
                                     message=f"{method} timeout")
        except SdkException as e:
            return TaskCommandResult(ok=False, code=e.code, message=str(e))
